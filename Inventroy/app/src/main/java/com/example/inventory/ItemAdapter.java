/*
 This java file holds the code that allows for the manipulation of Items in the inventory
 database. This code allows  user to add, modifies, and delete items as well as functionality some
  view functionality when selecting items. These are all important tools for managing an ever changing
  inventory database
*/
...
public class ItemAdapter extends RecyclerView.Adapter<ItemAdapter.ItemVH> {
    ...
    public void onBindViewHolder(@NonNull ItemVH holder, @SuppressLint("RecyclerView") int position) {
        ...
        holder.updateButton.setOnClickListener(v -> ((MainActivity)context).onUpdateItemClick(position, item.getId(), item.getTitle(), item.getQuantity(), item.getQuantityUnits()));
    }
